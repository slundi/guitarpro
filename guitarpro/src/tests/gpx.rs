...
#[test]
fn test_gpx_roundtrip_all_files() {
    use std::fs;
    let test_dir = "../test";
    let mut pass = 0;
    ...
    for entry in fs::read_dir(test_dir).unwrap() {
        ...
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            song1.read_gpx(&data).unwrap();
            let written1 = song1.write_gpx().unwrap();

            // Second read + write (idempotency check)
            let mut song2 = Song::default();
            song2.read_gpx(&written1).unwrap();
            let written2 = song2.write_gpx().unwrap();
    ...
    eprintln!(
        "GPX round-trip: {} pass, {} fail out of {}",
        pass,
        failures.len(),
        pass + failures.len()
    );
